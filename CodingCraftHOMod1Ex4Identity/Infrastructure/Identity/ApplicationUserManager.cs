...
namespace CodingCraftHOMod1Ex4Identity.Infrastructure.Identity
{
    ...
    public class ApplicationUserManager : UserManager<Usuario, int>
    {
        ...
        public static ApplicationUserManager Create(IdentityFactoryOptions<ApplicationUserManager> options, IOwinContext context = null) 
        {
            var dbContext = context.Get<ApplicationDbContext>() ?? new ApplicationDbContext();
            var manager = new ApplicationUserManager(new UserStore<Usuario, Grupo, int, UsuarioLogin, UsuarioGrupo, UsuarioIdentificacao>(dbContext));
            ...
            if (dataProtectionProvider != null)
            {
                manager.UserTokenProvider = 
                    new DataProtectorTokenProvider<Usuario, int>(dataProtectionProvider.Create("Coding Craft"));
            }
            return manager;
        }
    }
